...
class Solution {
    public int search(int[] nums, int target) {
        int lo = 0, hi = nums.length - 1, mid, num;
        while (lo <= hi) {
            mid = lo + (hi - lo) / 2; //[x,mid,y,z]; => left [x,mid] right [y, z] => hi = mid, lo = mid +1
            num = (nums[mid] >= nums[lo]) == (target >= nums[lo]) // nums[mid] and target at same side
                ? nums[mid]
                : (target < nums[lo]) ? Integer.MIN_VALUE : Integer.MAX_VALUE;
            if (num < target) 
                lo = mid + 1;
            else if (num > target)
                hi = mid;
